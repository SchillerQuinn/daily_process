...
function mousePressed() {
  if (all_particles.count < max_particles) {
    all_particles.addParticle(createVector(mouseX, mouseY), 20);
  }
}
...
function deviceShaken() {
  if (all_particles.count < max_particles) {
    //create 5 particles and make them slightly larger than normal
    all_particles.addParticle(createVector(random(0, imgg.width * scale), random(0, imgg.height * scale)), 10);
    all_particles.addParticle(createVector(random(0, imgg.width * scale), random(0, imgg.height * scale)), 10);
    all_particles.addParticle(createVector(random(0, imgg.width * scale), random(0, imgg.height * scale)), 10);
    all_particles.addParticle(createVector(random(0, imgg.width * scale), random(0, imgg.height * scale)), 10);
    all_particles.addParticle(createVector(random(0, imgg.width * scale), random(0, imgg.height * scale)), 10);
...
class Particle {
  constructor(location, scaleAmount) {
    this.location = createVector(location.x, location.y);
    this.isize = min(windowHeight / scaleAmount, windowWidth / scaleAmount);
    this.size = min(windowHeight / scaleAmount, windowWidth / scaleAmount);
  ...
  display() {
    ...
    fill(this.color.R, this.color.G, this.color.B, 255 - 255 * (this.size / this.isize));
